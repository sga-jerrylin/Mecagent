...
你的任务是**将代码匹配失败的3D零件与BOM表进行智能匹配**，目标是**100%匹配率**。
...
**优先级2：BOM代号对比**
- 检查3D零件名称是否包含BOM的`code`（`01.09.2549`格式）
- **示例**：
  - 零件名称：`01.09.2549-后座组件`
  - BOM code：`01.09.2549`
  - **判断**：完全匹配 → 置信度0.95
...
### 步骤4：置信度评估
根据匹配依据，评估置信度（**与代码匹配器保持一致**）：

**高置信度（0.90-0.98）：**
- **0.95-0.98**：BOM代号完全匹配（`01.09.2549`格式）
- **0.90-0.95**：产品代号完全匹配（`T-SPV250-Z602-01-01-Q355B`格式）
- **0.88-0.92**：产品代号部分匹配 或 标准号+规格+类型完全匹配

**中等置信度（0.75-0.90）：**
- **0.85-0.90**：产品代号前缀匹配 或 标准号+规格匹配
- **0.75-0.85**：尺寸规格匹配 或 规格+材料匹配

**低置信度（0.70-0.75）：**
- **0.70-0.75**：零件类型+材料匹配 或 名称相似度匹配
...
**⚠️ 重要：代码匹配器已经实现了以下3种匹配方法，AI匹配器必须使用相同的策略！**

### 方法1：BOM代号匹配（置信度0.95）
- **适用场景**：3D零件名称中包含BOM代号（`01.09.2549`格式）
- **示例**：
  - 3D零件名称：`01.09.2549-后座组件-漆后`
  - BOM代号：`01.09.2549`
  - **判断**：BOM代号完全一致 → **匹配！**

### 方法2：产品代号匹配（置信度0.90）✅ **核心方法**
- **适用场景**：3D零件名称中包含产品代号（`product_code`字段）
- **匹配策略**：
  1. **完全匹配**（置信度0.95-0.98）：
     - 3D零件名称：`T-SPV250-Z602-01-01-Q355B连接板`
     - BOM产品代号：`T-SPV250-Z602-01-01-Q355B`
     - **判断**：产品代号完全一致 → **匹配！**

  2. **部分匹配**（置信度0.85-0.90）：
     - 3D零件名称：`T-SPE-FL-01-SMS-01-Q235方形板-机加`
     - BOM产品代号：`T-SPE-FL-01-SMS-01-Q235`
     - **判断**：产品代号包含在零件名称中 → **匹配！**

  3. **前缀匹配**（置信度0.85-0.90）：
     - 3D零件名称：`JXG-T6×100×50-1120-Q355B矩形管`
     - BOM产品代号：`JXG-T6×100×50-1120-Q355B`
     - **判断**：产品代号前缀一致 → **匹配！**
